...
# set view
if [ "$error_chk" ];then
  echo "<h2>Oops please something must be wrong, please check  location_get.sh</h2>"

elif [ "$permission"  = "ro" ];then
  view="location_get_ro.html.def"

elif [ "$form_chk" = "urlenc" ];then
  if [ "$id" = "new" ];then
    view="location_get_new.html.def"
  else
    view="location_get_rw.html.def"
  fi
elif [ "$form_chk" = "multipart" ];then
  if [ "$id" = "new" ];then
    view="location_get_new_incf.html.def"
  else
    view="location_get_rw_incf.html.def"
  fi
fi
