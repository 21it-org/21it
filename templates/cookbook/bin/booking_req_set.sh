...
$DATA_SHELL databox:request.db action:set id:$id keys:$keys input_dir:../tmp/$session  > ../tmp/$session/result
...
if [ "$updated_id" ];then
  ...
  echo "<a href=\"./booking_req?req=get&id=$updated_id\"><p><b>YOUR LINK</b></p></a>" >> ../tmp/$session/message
else
...
# render HTML
# -----------------
cat ../descriptor/booking_req_set.html.def | sed "s/^ *</</g" \
