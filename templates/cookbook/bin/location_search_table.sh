...
# load post param
if [ -s /var/www/tmp/$session/table_command ];then
  table_command=`cat /var/www/tmp/$session/table_command`
fi
...
| $SED "/%%common_menu/d"\
| $SED "/%%table_menu/r /var/www/descriptor/common_parts/table_menu_${permission}" \
| $SED "/%%table_menu/d"\
