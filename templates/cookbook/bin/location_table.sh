...
# gen %%tag contents
$META get.tag:location{$databox} > ../tmp/$session/tags
for tag in `cat ../tmp/$session/tags`
do
 echo "<p><a href=\"./book_search?%%params&req=table&table_command=$tag\">#$tag&nbsp;</a></p>" > ../tmp/$session/tag &
done

# load permission
if [ ! "$user_name" = "guest" ];then
  permission=`$META get.attr:location/$user_name{permission}`
else
  permission="ro"
fi

# gen %%page_link contents
../bin/location_page_links.sh $page $pages "$table_command" > ../tmp/$session/page_link &
wait
...
if [ "$err_chk" ];then
  ...
  echo "<h2>Oops something must be wrong, please check location_table.sh</h2>"
  if [ "$session" ];then
    rm -rf ../tmp/$session
  fi
  exit 1
...
| sed "s/%%params/subapp=location\&session=$session\&pin=$pin\&databox=$databox/g"
