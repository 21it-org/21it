...
# gen %%tag contents
$META get.tag:book_search{$databox} > ../tmp/$session/tags
...
../bin/book_search_page_links.sh $page $pages "$table_command" > ../tmp/$session/page_link &
wait
...
if [ "$err_chk" ];then
  ...
  echo "<h2>Oops something must be wrong, please check  book_search_table.sh</h2>"
  if [ "$session" ];then
    rm -rf ../tmp/$session
  fi
  exit 1
